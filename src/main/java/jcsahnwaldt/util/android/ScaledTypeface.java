...
public class ScaledTypeface {
  ...
  public static ScaledTypeface createFromAsset(AssetManager mgr, String name, float factor, int tagKey) {
    if (name == null) return new ScaledTypeface(Typeface.DEFAULT, "system font", factor, tagKey);
    else return new ScaledTypeface(Typeface.createFromAsset(mgr, name), name, factor, tagKey);
  }
